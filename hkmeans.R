...
mywd = "C:/Users/Nick Morris/Downloads/Data-Science/Model-Scripts/Unsupervised/Hierarchical-K-Means"
...
if(workers > 1)
{
  ...
  cat("\n------------------------------------------------\n")
  cat("hierarchical kmeans clustering\n")
...
for(i in 1:tasks)
{
  ...
  pdf(paste0("hierarchical-kmeans-cluster-plot-", levels(dat$Model)[i], ".pdf"), width = 16, height = 10, paper = "special") 
  ...
  # set up a pdf file to capture the next graphic
  pdf(paste0("hierarchical-kmeans-silhouette-plot-", levels(dat$Model)[i], ".pdf"), width = 16, height = 10, paper = "special") 
...
# export dat & dat.dc
write.csv(cbind(dat, dat.dc), "hierarchical-kmeans-data.csv", row.names = FALSE)
